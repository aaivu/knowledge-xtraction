"""S3KG similarity: aligned WL score blended with SBERT mean-pool cosine."""
...
def _sbert_mean_cosine(kg1_triples, kg2_triples):
    ...
    emb1 = get_batch_embeddings(texts1).mean(axis=0)
    emb2 = get_batch_embeddings(texts2).mean(axis=0)

    return float(np.dot(emb1, emb2) / (np.linalg.norm(emb1) * np.linalg.norm(emb2) + 1e-8))
def calculate_s3kg_similarity(kg1_triples, kg2_triples, alpha=ALPHA):
    """
    Compute similarity between two KGs using the S3KG method.
...
def calculate_s3kg_similarity_score(kg1_triples, kg2_triples):
    """
    ...
    """
    score, _ = calculate_s3kg_similarity(kg1_triples, kg2_triples)
    return score
if __name__ == '__main__':
    print('=' * 60)
    print('S3KG - Semantic KG Similarity with SBERT Blend (alpha=0.5)')
    print('=' * 60)
    ...
    sim, info = calculate_s3kg_similarity(kg1, kg2_similar)
