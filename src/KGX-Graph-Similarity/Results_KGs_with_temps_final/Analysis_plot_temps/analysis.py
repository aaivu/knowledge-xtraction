...
For each QA instance q, the scored CSVs contain three KGs:
  - KG_LLM(q): KG extracted from the LLM answer
  - KG_gold(q): KG extracted from the gold/reference answer
  - KG_ctx(q): KG extracted from the supporting context
...
Dataset-level CUS is the mean of CUS(q) over all scored QA instances.
It reads CSV files from Results_KGs_with_temps_final and writes plots to
Results_KGs_with_temps_final/Analysis_plot_temps by default.
"""
...
def plot_comparison(df, out_dir):
    ...
    for col, dataset in enumerate(DATASETS):
        ...
        data = [metric_values(df, dataset, m, "gold_similarity") for m in MODELS]
        means = [v.mean() if len(v) else 0 for v in data]
        stds = [v.std() if len(v) else 0 for v in data]

        bars = ax_bar.bar(x, means, yerr=stds, capsize=5, color=colors, alpha=0.75, edgecolor="black")
        for bar, mean in zip(bars, means):
            ax_bar.text(bar.get_x() + bar.get_width() / 2, mean + 0.005, f"{mean:.3f}",
...
def plot_primary_table(df, out_dir):
    ...
    for model in MODELS:
        ...
        for dataset in DATASETS:
            ...
            row.extend([f"{gold.mean():.4f}" if len(gold) else "N/A",
                        f"{ctx.mean():.4f}" if len(ctx) else "N/A"])
        rows.append(row)
    cols = ["Model"] + [f"{d} {m}" for d in DATASETS for m in ["GoldSim", "CtxSim"]]
...
def plot_cus_table(df, out_dir):
    ...
    for dataset in DATASETS:
        for model in MODELS:
            ...
            if len(cus):
                rows.append([dataset, model, f"{gold.mean():.4f}", f"{ctx.mean():.4f}", f"{cus.mean():.4f}"])
...
def plot_grouped_bar(df, out_dir):
    ...
    for ax, (metric, title) in zip(axes, metrics):
        for i, model in enumerate(MODELS):
            ...
            ax.bar(x + i * width, means, width, label=model, color=COLORS[model], alpha=0.88)
...
def plot_winrate(df, out_dir):
    ...
    for dataset in DATASETS:
        ...
        for m1, m2 in combinations(MODELS, 2):
            left = sub[sub["model"] == m1][["id", "temperature", "gold_similarity"]]
            right = sub[sub["model"] == m2][["id", "temperature", "gold_similarity"]]
    ...
    table_figure(rows, cols, "Pairwise Win-Rate - GoldSim", out_dir, "6_winrate.png",
                 figsize=(14, 0.48 * len(rows) + 1.2))
...
def plot_distributions(df, out_dir):
    for dataset in DATASETS:
        ...
        parts = axes[2].violinplot([vals if len(vals) else [0] for vals in data],
    ...
    for dataset in DATASETS:
        for model in MODELS:
            ...
            if len(vals):
                rows.append([dataset, model, f"{vals.mean():.4f}", f"{np.median(vals):.4f}",
                             f"{vals.std():.4f}", f"{vals.min():.4f}", f"{vals.max():.4f}", str(len(vals))])
...
def plot_scatter(df, out_dir):
    ...
    for ax, dataset in zip(axes, DATASETS):
        for model in MODELS:
            sub = df[(df["dataset"] == dataset) & (df["model"] == model)]
            if not sub.empty:
                ax.scatter(sub["gold_similarity"], sub["context_similarity"], label=model,
                           color=COLORS[model], alpha=0.45, s=18, edgecolors="none")
...
def plot_significance(df, out_dir):
    ...
    for dataset in DATASETS:
        ...
        for m1, m2 in combinations(MODELS, 2):
            ...
            rows.append([dataset, m1, m2, f"{a.mean():.4f}", f"{b.mean():.4f}", w_text, p_text(mp)])
    cols = ["Dataset", "Model A", "Model B", "Mean A", "Mean B", "Wilcoxon p", "Mann-Whitney p"]
    print_table(rows, cols, "Statistical Significance Tests")
    table_figure(rows, cols, "Statistical Significance Tests", out_dir, "9_significance.png",
                 figsize=(17, 0.62 * len(rows) + 1.8))
            ...
            for model in MODELS:
                ...
                if np.isfinite(means).any():
                    ...
                    for temp, mean in zip(TEMPERATURES, means):
                        if np.isfinite(mean):
                            ax.annotate(f"{mean:.3f}", (temp, mean), textcoords="offset points",
    ...
    for dataset in DATASETS:
        for model in MODELS:
            for temp in TEMPERATURES:
                ...
                if len(gold):
                    rows.append([dataset, model, str(temp), f"{gold.mean():.4f}",
                                 f"{ctx.mean():.4f}", f"{cus.mean():.4f}"])
    cols = ["Dataset", "Model", "Temperature", "Mean GoldSim", "Mean CtxSim", "Mean CUS"]
    print_table(rows, cols, "Temperature Effect - Mean Scores")
    table_figure(rows, cols, "Effect of Temperature on S3KG Scores", out_dir, "12_temperature_table.png",
                 figsize=(16, 0.4 * len(rows) + 1.0))
...
def plot_eval_table(df, out_dir, temperature=0.0):
    ...
    print_table(rows, cols, title)
    table_figure(rows, cols, title + "\n* = best per metric per dataset", out_dir,
                 f"13_eval_table_t{temperature}.png", figsize=(10, 0.4 * len(rows) + 1.2))
...
def main():
    ...
    parser.add_argument("--outdir", default=OUT_DIR, help="Directory to write plot images")
