"""
KRPOMultiAlign — simultaneous N-graph extraction with KRPO feedback loop.

One LLM call extracts all N graphs jointly from N paragraphs with label alignment.
Then iterative KRPO loop: triplet→text (NLI grounding) → score each graph against its
own paragraph → combined = avg(scores) → backward_eval → backward_pred → optimizer →
accept trial prompt if better. Early stop at combined ≥ 0.85.
"""
...
# Base extraction template — {n}, {graph_list}, {text_block} filled at runtime
...
def krpo_extract(
    ...
    Args:
        ...
        paragraphs:  List of N paragraph strings (N ≥ 1).
        max_rounds:  Maximum KRPO refinement rounds (default 3).
        row_id:      Identifier for logging (optional).

    Returns:
        dict: {"graph_1": [...], "graph_2": [...], ...} — one list of triplets per paragraph.
    """
    n = len(paragraphs)
    ...
    for rnd in range(max_rounds):
        ...
        if not any(graphs):
            log.warning(f"  Row {row_id} round {rnd+1}: no triplets extracted — skipping round")
            continue
