...
_MULTI_EXTRACTION_BASE = (
    "Your task is to extract a COMPLETE and EXHAUSTIVE set of knowledge graphs from {n} text(s). "
    "Each graph must capture EVERY single fact stated in its text — missing information is a critical failure. "
    "Extract all {n} graphs simultaneously so that entity and relation labels are CONSISTENT ACROSS ALL GRAPHS.\n\n"
    "EXTRACTION RULES — READ CAREFULLY:\n"
    "- COMPLETENESS IS THE TOP PRIORITY. Every sentence, clause, and phrase must produce at least one triplet.\n"
    "- Extract EVERY fact: attributes, quantities, dates, locations, roles, actions, relationships, and properties.\n"
    "- Do NOT skip supporting or background details — extract them too.\n"
    "- Replace ALL pronouns (he, she, it, they, his, her, its) with the actual entity name.\n"
    "- Split compound sentences into as many triplets as needed — one triplet per fact.\n"
    "- The SAME real-world entity MUST use the SAME label in ALL graphs "
    "(e.g., always \"Marie_Curie\", never \"Curie\" or \"M._Curie\").\n"
    "- The SAME relationship type MUST use the SAME label in ALL graphs "
    "(e.g., always \"born_in\", never \"born_at\").\n"
    "- Use underscores for multi-word entities and relations "
    "(e.g., \"New_York\", \"Nobel_Prize_in_Physics\", \"born_in\").\n"
    "- Keep relation labels concise and reusable (e.g., \"located_in\", \"part_of\", \"causes\").\n"
    "- Do NOT infer or assume anything not explicitly stated in the text.\n"
    "- STRICT ISOLATION: Each graph must contain ONLY facts from its OWN text. "
    "NEVER borrow, copy, or derive triplets from another text's content — even if the texts share entities.\n"
    "- Do NOT include duplicate triplets within a single graph.\n\n"
    "OUTPUT FORMAT (output ONLY these {n} line(s), no extra text, no explanation):\n"
    "{graph_list}\n\n"
    "EXAMPLE:\n"
    "Text 1: Marie Curie was born in Warsaw on November 7, 1867. She was a physicist and chemist. "
    "She won the Nobel Prize in Physics in 1903 and the Nobel Prize in Chemistry in 1911.\n"
    "Text 2: Marie Curie, a Polish scientist, conducted research in Paris alongside her husband Pierre Curie. "
    "Their work focused on radioactivity. She was the first woman to win a Nobel Prize.\n"
    "graph_1: [['Marie_Curie', 'born_in', 'Warsaw'], ['Marie_Curie', 'birth_date', 'November_7_1867'], "
    "['Marie_Curie', 'occupation', 'physicist'], ['Marie_Curie', 'occupation', 'chemist'], "
    "['Marie_Curie', 'won', 'Nobel_Prize_in_Physics'], ['Nobel_Prize_in_Physics', 'awarded_in', '1903'], "
    "['Marie_Curie', 'won', 'Nobel_Prize_in_Chemistry'], ['Nobel_Prize_in_Chemistry', 'awarded_in', '1911']]\n"
    "graph_2: [['Marie_Curie', 'nationality', 'Polish'], ['Marie_Curie', 'occupation', 'scientist'], "
    "['Marie_Curie', 'conducted_research_in', 'Paris'], ['Marie_Curie', 'collaborated_with', 'Pierre_Curie'], "
    "['Pierre_Curie', 'relation_to', 'Marie_Curie_husband'], "
    "['Marie_Curie', 'research_focus', 'radioactivity'], ['Pierre_Curie', 'research_focus', 'radioactivity'], "
    "['Marie_Curie', 'distinction', 'first_woman_Nobel_Prize_winner']]\n\n"
    "{text_block}"
)
